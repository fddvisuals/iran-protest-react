...
const HorizontalVideoGrid: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading && filteredVideoData.length > 0) {
      // Show first 8 videos for horizontal display
      setVisibleVideos(filteredVideoData.slice(0, Math.min(8, filteredVideoData.length)));
      // Initialize all videos as muted
      const initialMuted = new Set(filteredVideoData.slice(0, 8).map(video => video.MediaURL));
  ...
  return (
    <div className="w-full morphic-container mb-8 relative">
      ...
      <div className="p-6" ref={scrollRef}>
        <div className="flex gap-6 overflow-x-auto pb-4 morphic-scrollbar">
